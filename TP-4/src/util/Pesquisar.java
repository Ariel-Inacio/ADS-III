...
public class Pesquisar {
    ...
    // Método para pesquisar um topico sobre o filme/serie
    public static String pesquisarTopico(int selecao, Filmes filme) {

        return switch (selecao) {
            case 1 -> filme.getNOME();
            case 2 -> filme.getDIRETOR();
            case 3 -> filme.getPAIS();
            case 4 -> filme.getANO_ADI().format(DateTimeFormatter.ofPattern("dd/MM/yyyy")); // LocalDate para String
            case 5 -> filme.getANO_LAN().toString(); // Year para String
            case 6 -> filme.getCLSSIFICACAO();
            case 7 -> filme.getGENERO();
            default -> null; // Retorna null se a seleção for inválida
        };
