...
public class HashExtensivel<T extends RegistroHashExtensivel<T>> {
  ...
  public void close() throws IOException {
    ...
    Path pathArqDir = Paths.get(nomeArquivoDiretorio);
    Path pathArqCesto = Paths.get(nomeArquivoCestos);
    if (Files.exists(pathArqDir)) {
      Files.delete(pathArqDir);
    }
    if (Files.exists(pathArqCesto)) {
      Files.delete(pathArqCesto);
    }
  }
}
